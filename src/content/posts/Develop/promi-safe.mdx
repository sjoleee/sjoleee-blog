...
다만, `fetchUser`가 페칭과 검증의 두 역할을 하고 있다는 점이 다소 아쉽습니다.  
...
그러나 실제 서비스에서는 수많은 API가 존재하고, 그에 맞는 수많은 데이터 타입이 있을 거에요.  
그렇다면 `validateUser` 같이 데이터를 검증하는 함수가 수십개 생기겠죠?  
...
유저는 메인 페이지에서 불필요하게 `ErrorFallback`을 봐야만 할거에요.
...
검증 시 서비스 이용에 필요한 영역이 제대로 동작할 수 없는 문제가 발생했을 때가 아니라면 굳이 터트릴 필요는 없다고 생각해요.  
...
저희 회사는 google Docs로 서버 스펙을 공유받는 상황이라 클라이언트단에서 구축할 수 있는 최소한의 안전 장치가 필요했답니다🥲)
