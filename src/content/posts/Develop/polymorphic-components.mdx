...
이런 컴포넌트를 `다형성을 가지는 컴포넌트`라고 부릅니다.  

'많은(poly)'과 '형태(morph)'라는 그리스어에서 유래한 이 용어는, **하나의 객체가 여러 가지 형태를 가질 수 있다**는 개념을 나타냅니다.  
객체 지향에서 중요한 개념 중 하나인 OCP와 연관이 있을 것 같아요.  

OCP, 개방-폐쇄 원칙에서 말하는 개방은 확장에 대한 개방, 폐쇄는 수정에 대한 폐쇄를 의미합니다.  
만약 우리가 `<Button as="a">` 처럼 사용한다면, 버튼에 새로운 기능(`a` 태그가 갖는 속성)을 추가할 때 기존 코드를 수정하지 않아도 되겠죠.  
또한, `as` prop은 구체적인 구현을 숨기고, 필요한 동작만을 외부에 노출하고 있으니 추상화, 캡슐화도 적용되고 있습니다.  

컴포넌트 관점에서 본다면, `as` prop은 컴포넌트의 재사용성을 높여주고, 유연성을 제공해 줍니다.  
예시처럼 버튼이 다양한 형태로 사용될 수 있으니, 재사용성은 당연히 높아지겠죠?🤗  
